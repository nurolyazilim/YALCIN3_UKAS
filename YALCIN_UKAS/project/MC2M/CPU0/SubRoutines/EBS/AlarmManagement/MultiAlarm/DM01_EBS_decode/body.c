    ...
    for ( i = 0; i < DTC_COUNT; i ++ )
    {
        ...
        EBS_dtcSpnEngine[i+1] = SPN; //SPN listesi
        EBS_dtcFmiEngine[i+1] = FMI; //FMI listesi
        EBS_dtcSpnEngine[0]   = i+1; //Kac adet alarm var? 

    EBS_dataMatrix[i][0] = i+1;
    EBS_dataMatrix[i][1] = SPN;
    EBS_dataMatrix[i][2] = USINT_TO_UDINT(FMI);
    EBS_dataMatrix[i][3] = USINT_TO_UDINT(OC);
