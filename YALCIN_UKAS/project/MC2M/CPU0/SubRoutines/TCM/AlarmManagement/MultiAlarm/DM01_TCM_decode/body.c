    ...
    for ( i = 0; i < DTC_COUNT; i ++ )
    {
        ...
        TCM_dtcSpnEngine[i+1] = SPN; //SPN listesi
        TCM_dtcFmiEngine[i+1] = FMI; //FMI listesi
        TCM_dtcSpnEngine[0]   = i+1; //Kac adet alarm var? 

    TCM_dataMatrix[i][0] = i+1;
    TCM_dataMatrix[i][1] = SPN;
    TCM_dataMatrix[i][2] = USINT_TO_UDINT(FMI);
    TCM_dataMatrix[i][3] = USINT_TO_UDINT(OC);
